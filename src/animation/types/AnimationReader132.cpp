	...
	for (uint32_t x = 0; x < extHeader.skeletons.size(); x++)
	{
		...
		for (uint32_t y = 0; y < curSkel.actions.size(); y++)
		{
			...
			for (size_t x = 0; x < curSkel.bones.size(); x++)
			{
				...
				bodyReader->readSingleArray(&initTransform.rot[x][0], 4);
				bodyReader->readSingleArray(&initTransform.pos[x][0], 3);
				bodyReader->readSingleArray(&initTransform.scale[x][0], 3);
			}
			...
			bodyReader->readSingleArray(timeScales.data(), curActionHead.frameCount);
